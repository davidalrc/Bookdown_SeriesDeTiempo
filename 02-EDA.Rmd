...
```{python}
pd.plotting.lag_plot(ahuyama_ts,lag=1)
plt.title("Lag plot serie de precios promedios mayoristas semanales Ahuyama" )
plt.show()
plt.close()
```
A través del lag plot con un rezgo es posible apreciar que los datos de esta serie de tiempo exhiben un patrón irregular en donde algunos valores toman una tendencia lineal y otros simplemente toman una distribución mucho más aleatoria creando una nube de puntos, en términos muy generales el aspecto de este lag plot da pie para decir que la serie tiene un componente evidentemente aleatorio y otro que muestra claras señales de autocorrelación (sección superior derecha del gráfico), teniendo en cuenta que cuando los datos presentan comportamiento periodico este tipo de gráfica toma comportamientos circulares o sinusuoidales, es posible inferir que efectivamente esta serie de tiempo no tiene estacionariedad clara.
